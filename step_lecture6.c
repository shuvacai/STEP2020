...
// 1. Used Best-fit to find the optimal free space
// 2. Coalease adjacent free blocks every time a new block is added to the free list
...
void my_add_to_free_list(simple_metadata_t* metadata) {
  ...
  void* ptr = metadata + 1;
  void* metadata_new = ((char*)ptr + metadata->size);
  simple_metadata_t* prev = NULL;
  
  while(metadata){
      prev = metadata;
      metadata = metadata->next;
      if (metadata == metadata_new){
          simple_metadata_t* head = simple_heap.free_head;
          //Coalease adjacent free blocks
          head->size = head->size + metadata->size + sizeof(simple_metadata_t);
          prev->next = metadata->next;
      }
  }
}
...
void* my_malloc(size_t size) {
  ...
  // BEST FIT: Finds the smallest block it can fit
  simple_metadata_t* best_block = metadata;
  ...
  while (best_block) {
    if (best_block->size == size){
      ...
      metadata= best_block;
      break;  
    }else if (best_block->size < metadata->size && best_block->size > size){
      prev = best_prev;
      metadata= best_block;
    }
